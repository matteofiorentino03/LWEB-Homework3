...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($errore === "") {
        $xmlFile = __DIR__ . '/xml/centrocampisti.xml';
        ...
        if (file_exists($xmlFile)) {
            ...
            $root = $dom->createElement("centrocampisti");
            $dom->appendChild($root);
        }

        $root = $dom->documentElement;

        $centrocampista = $dom->createElement("centrocampista");

        $centrocampista->appendChild($dom->createElement("ID_giocatore", $id_giocatore));
        $centrocampista->appendChild($dom->createElement("gol_fatti", $gol_fatti));
        $centrocampista->appendChild($dom->createElement("assist", $assist));
        $centrocampista->appendChild($dom->createElement("ammonizioni", $ammonizioni));
        $centrocampista->appendChild($dom->createElement("espulsioni", $espulsioni));
        $centrocampista->appendChild($dom->createElement("ruolo", $ruolo_cm));

        $root->appendChild($centrocampista);
        $dom->save($xmlFile);
